...
function get_data3(starti, ende){
    ...
    var aj = $.ajax({
        type: "POST",
        url: "/functions/php/get_monat.php",
        data: postdata,
        dataType: 'json',
        cache: false,
        async: false
...
function transform_date(dat){
    ...
    var jahr  =  dataSplit[1];
    var tag   =  '01';

    var newDate=monat+"/"+tag+"/"+jahr;

    var timestamp = new Date(newDate);    
    var timestamp = new Date(newDate).getTime();
    var tagback   = timestamp - (1 * 24 * 3600);
    
    var datum = new Date(tagback);

    var yyyy = datum.getFullYear().toString();                                    
    var mm   = (datum.getMonth()+1).toString();          
    var dd   = datum.getDate().toString();             
...
//
  function chkForm(formu)
  {
    ...
    if (newstart == "" || newende == "")
    {
        alert("Bitte Start- und Endemonat eingeben!");
...
function processing(formu) {
  ...
  { 
   ...
   var durchschnitt = "Durchschnittswert:" + " " + daten[0] + " " + "kw/h pro Monat"; 
